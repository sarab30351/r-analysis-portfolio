...
### *PROJECT UPDATE* 02.09: Following proportional hazards diagnostics, the full-follow-up relapse-free survival model was replaced by interval-specific Cox models. The initial intervals were 0–2, 2–5, 5–10, and beyond 10 years. Because the 2–5-year interval retained evidence of time-varying molecular-subtype associations, it was divided at its neutral midpoint of 3.5 years. The final RFS analysis therefore uses five intervals: 0–2, 2–3.5, 3.5–5, 5–10, and beyond 10 years. The 3.5-year boundary was selected after reviewing the diagnostics; it was not prespecified, estimated as an optimal change point, or treated as an externally established clinical cutoff.

### *PROJECT UPDATE* 30.08: There have been minor changes to continuous variable modeling. Age and NPI were retained as continuous variables and modeled using natural cubic splines. The original analysis plan proposed four knots for both predictors. During model development, age retained four knots at the 5th, 35th, 65th, and 95th percentiles, while NPI used three knots at the 5th, 50th, and 95th percentiles. The reduced NPI specification was used because its middle quantiles were tightly clustered around 4, so two internal knots would have added flexibility within a very narrow range. The same percentile specification was used for OS and RFS, with numerical knot values calculated from each analytical cohort.

### *PROJECT UPDATE* 30.08: Additional OS robustness analyses were completed in Scripts `13`-`14`. Each interval-specific model estimated ten regression coefficients, with 119-381 deaths across the four intervals. Subtype-specific event counts, confidence interval widths, and leave-one-out influence analyses were examined when evaluating estimate stability. No single-patient deletion changed a subtype hazard ratio by 10% or more.

### *PROJECT UPDATE* 23.08: The primary overall survival analysis is complete. The initial full follow-up Cox model showed substantial violations of the proportional hazards assumption, so the final analysis used interval-specific Cox models for 0-2, 2-5, 5-10, and beyond 10 years. Each model adjusted for age at diagnosis and Nottingham Prognostic Index using natural cubic splines and was stratified by source cohort. Holm adjustment was applied separately to the four global subtype tests and to the 20 comparisons between individual subtypes and Luminal A across the four follow-up intervals. Age- and NPI-stratified sensitivity analyses produced similar estimates, with all changes in subtype hazard ratios below 10%. The final tables, proportional hazards diagnostics, sensitivity analyses, and interval-specific hazard ratio figures are generated reproducibly by scripts `06`–`14`.
